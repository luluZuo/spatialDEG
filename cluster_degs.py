...
def find_cluster_degs(
    ...
    the difference between the percentages of buckets expressing the gene in the 
    ...
    ----------
        ...
        method: 'str'(default:'all')
            This method is to choose the difference expression genes test method 
            between test group and other groups one by one or combine them 
    ...
    #n_buckets, n_genes = X_data.shape
...
def find_spatial_cluster_degs(
    ...
    ----------
        ...
        ratio_thresh:'float'(defult=0.5)
            For each non-test group, if more than 50% (default) of its spots are in the 
            neighboring set, this group is then selected as a neighboring group.
